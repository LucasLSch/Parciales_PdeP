{-
  Repetición de funciones mapAlgo en vez de hacer funciones maś específicas.
  Nota: 7
-}
...
{-
  Para no repetir (mapEpiteto (ganaEpiteto "El mitico")) podés definir una función cambiarEpiteto
  que haga la lógica (mapEpiteto (ganaEpiteto unEpiteto)).
  Lo mismo para mapArtefactos pero definiendo una función agregarArtefacto.
-}
...
{-
  En vez que ganaEpiteto y ganaArtefacto solo llamen a const y (:) respectivamente, podrías
  definir funciones con más lógica (como cambiarEpiteto y agregarArtefacto).
-}
...
{-
  Para no repetir (mapReconocimiento (+ unValor)) podés definir una función ganarReconocimiento.
-}
...
{-
  Podés hacer los mapArtefactos en uno solo con composición:
  mapArtefactos (ganaArtefacto relampagoDeZeus . desecharRarezaQue (< 1000) . multiplicaRareza 3)
-}
...
{-
  Idem ganaEpiteto y ganaArtefacto. Sería mejor tener una función agregue las tareas
  directamente al héroe.
-}
